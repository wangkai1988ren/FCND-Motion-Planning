...
from planning_utils import point, collinearity_check, prune_path, pick_uncrossed
...
class MotionPlanning(Drone):
    ...
    def local_position_callback(self):
        if self.flight_state == States.TAKEOFF:
            ...
            if np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 1:#1.0:
        ...
        if self.flight_state == States.LANDING:
            if self.global_position[2] - self.global_home[2] < 0.1:
                if abs(self.local_position[2]) < 0.01:
                    print(self.local_position) ##test
    ...
    def plan_path(self):
        ...
        for p in path:
            ...
            if len(waypoints)>1:
                waypoints[-1][3] = np.arctan2((waypoints[-1][1]-waypoints[-2][1]),(waypoints[-1][0]-waypoints[-2][0]))
        # Set self.waypoints
